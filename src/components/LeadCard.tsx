        ...
        <div className='pt-20'>
            <div className='grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 lg:gap-[60px] gap-6 justify-center'>
                {visibleCards.map((obj: LeadCardData, i: number) => (
                    <div key={i} className=''>
                        <Link href={`/card/${obj ? obj.title?.toLowerCase()?.replace(/ /g, '-') : '404'}`} className={`${obj.bgImage} bg-full !bg-no-repeat w-full lg:max-w-[440px] 2xl:max-w-full min-h-[551px] relative xl:p-7 p-5 flex hover:scale-110 transition-all ease-linear duration-300`}>
                            <div className='flex items-end mx-auto'>
                                <div className=' md:min-h-[320px] min-h-[250px] max-w-[382px] w-full bg-white rounded-3xl xl:p-[35px_28px_29px_28px] p-4 flex flex-col justify-between'>
                                    ...
                                    <div><button className='font-semibold md:text-base text-sm leading-[20px] text-black'>Read More</button></div>
                                </div>
                            </div>
                        </Link>
                    </div>
